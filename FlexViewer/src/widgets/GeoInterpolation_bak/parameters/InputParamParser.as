...
public class InputParamParser extends BaseParamParser
{
    ...
    private function parseMultiValueChoiceList(choiceListXML:XMLList):Array
    {
        ...
        {
            //读取xml文件中关于插值方法的参数的定义
			
			//定义一种插值方法对应的参数对象集合
			var params:ArrayCollection = new ArrayCollection();
			
			for each(var param:XML in choice.param){
				params.addItem({"name":String(param.@name),"label":String(param.@label),"value":String(param.@defaultvalue),"defaultvalue":String(param.@defaultvalue)});
			}
			trace(choice.@isGeoInterpolation);
			var geoFlag:Boolean = Boolean(choice.@isGeoInterpolation == "true");
			choiceList.push(new MultiValueItem(choice.@value,choice.@item2,params,geoFlag));
        }
