...
///////////////////////////////////////////////////////////////////////////
package widgets.GeoInterpolation.supportClasses
{
...
import widgets.GeoInterpolation.parameters.*;
...
public class GPParamHandler
{
	...
	public function setCrossValidateColumns(flag:Boolean,columnName:String):void
	{
		
		var recordSetParam:RecordSetParameter = findOutputParamByName("CrossValidationOutput_View") as RecordSetParameter;
		
		var list:ArrayCollection;
		if(recordSetParam)
			list = new ArrayCollection();
			if(flag && columnName){
				list.addItem({"name":"OBJECTID","label":"ID"});
				list.addItem({"name":columnName,"label":"实际值"});
				list.addItem({"name":"RASTERVALU","label":"预测值"});
				list.addItem({"name":"error","label":"误差"});
			}else{

				list.addItem({"name":"FID","label":"ID"});
				list.addItem({"name":"Measured","label":"实际值"});
				list.addItem({"name":"Predicted","label":"预测值"});
				list.addItem({"name":"Error","label":"误差"});
			}
			recordSetParam.fields = list;
			
	}
	
	public function addOutPutParam(flag:Boolean):void
	{
		if(flag){//地统计插值
			
			
			var b:IGPParameter = findOutputParamByName("output_value");
			var c:IGPParameter = findOutputParamByName("output_value_json");
			if(b)outputParams.splice(outputParams.indexOf(b),1);
			if(c)outputParams.splice(outputParams.indexOf(c),1);
			
			var a:IGPParameter = findOutputParamByName("output_Statistics");
			if(!a){
				var gpParam1:IGPParameter = new RecordSetParameter();
				
				gpParam1.label = "交叉验证综合结果";
				gpParam1.name = "output_Statistics";
				gpParam1.visible = false;
				
				outputParams.push(gpParam1);
			}

		}else{
			
			//是否需要清空？？？？
			var b:IGPParameter = findOutputParamByName("output_Statistics");
			if(b)outputParams.splice(outputParams.indexOf(b),1);
			
			
			var a:IGPParameter = findOutputParamByName("output_value");
			if(!a){
				//第一参数
				var gpParam1:IGPParameter = new StringParameter();
				gpParam1.label = "交叉验证综合结果";
				gpParam1.name = "output_value";
				gpParam1.visible = false;
				
				outputParams.push(gpParam1);
			}
			var b:IGPParameter = findOutputParamByName("output_value_json");
			if(!b){
				//第二个参数
				var gpParam2:IGPParameter = new StringParameter();
				gpParam2.label = "交叉验证综合结果json";
				gpParam2.name = "output_value_json";
				gpParam2.visible = false;
				
				outputParams.push(gpParam2);
			}
		}
